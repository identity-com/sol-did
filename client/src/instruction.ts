...
export function initialize(solidAccount: PublicKey, authority: PublicKey): TransactionInstruction {
...
export function write(solidAccount: PublicKey, authority: PublicKey, offset: BN, solidData: Uint8Array): TransactionInstruction {
  const keys: AccountMeta[] = [
    {pubkey: solidAccount, isSigner: false, isWritable: true},
...
SCHEMA.set(SolidInstruction, {kind: 'enum', field: 'enum', values: [
  ['initialize', Initialize],
  ['write', Write],
  ['setAuthority', SetAuthority],
  ['closeAccount', CloseAccount],
