...
export class DidAccountSizeHelper {
  ...
  static getVerificationMethodSize(verificationMethod: VerificationMethod | undefined): number {
  ...
  getDidAccountSize(): number {
    return 1 // version
      ...
      + 4 + this.didAccount.verificationMethods.reduce((acc, cur) => acc + DidAccountSizeHelper.getVerificationMethodSize(cur), 0) // verification_methods
      // + 4 + self.verification_methods.iter().fold(0, | accum, item| { accum + item.size() }) // verification_methods
      + 4 + this.didAccount.services.reduce((acc, cur) => acc + DidAccountSizeHelper.getServiceSize(cur), 0) // services
      // + 4 + self.services.iter().fold(0, | accum, item| { accum + item.size() }) // services
      + 4 + this.didAccount.nativeControllers.length * 32// native_controllers
      // + 4 + self.native_controllers.len() * 32 // native_controllers
      + 4 + this.didAccount.otherControllers.reduce((acc, cur) => acc + 4 + getBinarySize(cur), 0) // other_controllers
