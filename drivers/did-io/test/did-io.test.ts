...
describe('did-io integration', () => {
  beforeAll(async () => {
    ...
    const payerAccount = await SolanaUtil.newAccountWithLamports(connection, 10000000);
  ...
  it('creates a did on devnet', async () => {
    ...
    expect(document.verificationMethod[0].publicKeyBase58).toEqual(owner.publicKey.toBase58());
  });
});
